...
import com.example.hazem.phonebook.Features.Activity.LogsScreen;
...
public abstract class CallLogTouchListener implements AbsListView.OnTouchListener {
    private int padding = 0;
    private int initialx = 0;
    private int currentx = 0;
    private boolean normalClick=false;
    private LogsAdapter.LogsHolder holder;
    Activity activity;
    LogModel model;
    public CallLogTouchListener(LogsAdapter.LogsHolder holder, Activity activity, LogModel model)
    {
        ...
        if (holder != null) {
            if (padding == 0) {
                ...
                if (holder.running) {
                    ...
                    intent.setData(Uri.parse("tel:" + model.getCallerNumber()));
    ...
    void onItemClick()
    {
        Intent intent=new Intent(activity, LogsScreen.class);
        intent.putExtra(NavConstants.LogModelKey,model);
        activity.startActivity(intent);
    }
}
